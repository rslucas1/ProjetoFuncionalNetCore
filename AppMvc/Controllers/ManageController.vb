...
Public Class ManageController
    ...
    Public Async Function Index(message As System.Nullable(Of ManageMessageId)) As Task(Of ActionResult)
        ViewData!StatusMessage = If(message = ManageMessageId.ChangePasswordSuccess, "Sua senha foi alterada.", If(message = ManageMessageId.SetPasswordSuccess, "Sua senha foi definida.", If(message = ManageMessageId.SetTwoFactorSuccess, "Seu provedor de autenticação de dois fatores foi definido.", If(message = ManageMessageId.[Error], "Ocorreu um erro.", If(message = ManageMessageId.AddPhoneSuccess, "Seu número de telefone foi adicionado.", If(message = ManageMessageId.RemovePhoneSuccess, "Seu número de telefone foi removido.", ""))))))
    ...
    Public Async Function AddPhoneNumber(model As AddPhoneNumberViewModel) As Task(Of ActionResult)
        ...
        If UserManager.SmsService IsNot Nothing Then
            Dim message = New IdentityMessage() With {
                ...
                .Body = "Seu código de segurança é:" & Convert.ToString(code)
            }
    ...
    Public Async Function ManageLogins(message As System.Nullable(Of ManageMessageId)) As Task(Of ActionResult)
        ViewData!StatusMessage = If(message = ManageMessageId.RemoveLoginSuccess, "O login externo foi removido.", If(message = ManageMessageId.[Error], "Ocorreu um erro.", ""))
        Dim userInfo = Await UserManager.FindByIdAsync(User.Identity.GetUserId())
